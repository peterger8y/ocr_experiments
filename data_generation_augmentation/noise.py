...
def slant(im, ratio = random.choice([30,40])):
...
def margin(im):
    boundary1 = random.randint(0, 250)
    boundary2 = random.randint(boundary1+1, 256)
...
def uniform_lineup(im):
    num_lines = random.choice([10, 25, 50])
    spacing = im.shape[0] / num_lines
    for line in range(im.shape[0]):
        if line % spacing == 0:
            im[line, :, :] = random.randint(0, 50)
    return im
