...
from utils.noise import lineup, noisy
...
def augment_transform(image, label):
    ...
    if random.choice([True, False]):
      image = 255 - image
      image = np.uint8(image) - random.randint(0, 100) 
      image = 255 - image
    if random.choice([True, False]):
      image = 255 - image
      image = np.uint8(image) + random.randint(0, 100)
      image = 255 - image
    func_directory = {1: lineup, 2: noisy}
    ...
    for x in func_choices:
      image = func_directory[x](image)             
    s = [i+1 for i, x in enumerate(os.listdir('/content/drive/MyDrive/OrigamiNet/distorted_results'))]
    s = s+[0]
    alpha = max(s) + 1
    cv2.imwrite('/content/drive/MyDrive/OrigamiNet/distorted_results/'+str(alpha)+'.png', np.uint8(image))
    return transform(image, augmented_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--checkpoint_dir", default="/content/drive/MyDrive/handwritten-text-recognition-for-apache-mxnet-master/model_checkpoint",
    ...
    train_ds = IAMDataset("form_bb", credentials=('petergeraghty57@gmail.com', 'bewros-zorNaz-5cicjo'), output_data="bb", output_parse_method=detection_box, train=True)
    print("Number of training samples: {}".format(len(train_ds)))

    test_ds = IAMDataset("form_bb", credentials=('petergeraghty57@gmail.com', 'bewros-zorNaz-5cicjo'), output_data="bb", output_parse_method=detection_box, train=False)
