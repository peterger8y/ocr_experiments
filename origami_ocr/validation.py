...
from origami_ocr.utils import Averager
...
def validation(model, criterion, evaluation_loader, converter, opt, parO):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        labels = [labels]
        image_tensors = np.expand_dims(image_tensors, axis=1)
    ...
    nelms = float(len(evaluation_loader))#float(len(evaluation_loader.dataset))
